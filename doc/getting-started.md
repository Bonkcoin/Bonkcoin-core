...
This tutorial will help you to go through the basics to use Boncoin Core after you completed the [installation instructions](/INSTALL.md). You now have `boncoind` or `boncoin-qt` executables available to run a node, and `boncoin-cli`/`boncoin-tx` tools to help you transact BONC.
...
> ```console
> satoshi:~$ /path/to/boncoin-cli [arguments ...]
> ```

### Table of contents

1. [Starting a boncoin node](#starting-a-boncoin-node)
...
## Starting a Boncoin node

To start your node, you can run an headless server using `boncoind`:
```console
satoshi:~$ boncoind -daemon
```

Or you can use the Graphical User Interface (GUI), `boncoin-qt`:
```console
satoshi:~$ boncoin-qt
```
...
Your node is now running and starts with a *synchronization process* that downloads the entire blockchain from other nodes. This operation will take many hours to complete, but you are now part of the Boncoin network!

> **Note:** The rest of this guide assumes the use of an headless node. The RPC server is not exposed with `boncoin-qt` until you activate the `-server` option as a startup argument, but inside the GUI application, you can use all the commands explored below (without `boncoin-cli`) by going to `Help -> Debug window` and inside the popup window selecting the tab `Console`.

## Introduction to the JSON-RPC API

Boncoin Core exposes a JSON-RPC interface that allows you to request information about the network, blockchain and individual transactions, send transactions to the networks and manage your wallet.

The Boncoin Core installation provides the `boncoin-cli` tool to interact with the JSON-RPC from the command line, and the interface is exposed over HTTP on port `15612`, so that other tools and libraries can interact with it.

To have an overview of the available commands, use the `help` command:

```console
#List all commands
satoshi:~$ boncoin-cli help

#Get help for a specific command
satoshi:~$ boncoin-cli help COMMAND
```
...
By default, the Boncoin Core software will automatically create an address for you and securely store the private key in the wallet file.

You can list wallet addresses using `getaddressesbyaccount`:

```console
satoshi:~$ boncoin-cli getaddressesbyaccount ""
[
...
```console
satoshi:~$ boncoin-cli getnewaddress
...
```console
satoshi:~$ boncoin-cli backupwallet /path/of/wallet/backup
```

**Tip:** Boncoin addresses start with the letter `P`.
...
```console
#Syntax
satoshi:~$ boncoin-cli getbalance "*" minconf
```
...
```console
satoshi:~$ boncoin-cli getbalance "*" 5
421.552000
```
...
```console
#Syntax
satoshi:~$ boncoin-cli sendtoaddress address amount

#Example
satoshi:~$ boncoin-cli sendtoaddress nWSYUqtimF7B6qW4GBdczaG6jvqKutS1Nh 420
```
...
```console
#Syntax
satoshi:~$ boncoin-cli listunspent minconf maxconf '["address", ...]'

#Example
satoshi:~$ boncoin-cli listunspent 1 9999999 '["nnJDY1xCRgWQc7vBXHUPMPsEynuZW23Y3P"]'
[
  {
...
satoshi:~$ boncoin-cli createrawtransaction "$utxos_to_use" '{"address":amount, ...}'
...
satoshi:~$ boncoin-cli createrawtransaction "$utxos_to_use" '{"nWSYUqtimF7B6qW4GBdczaG6jvqKutS1Nh":69, "nnJDY1xCRgWQc7vBXHUPMPsEynuZW23Y3P": 30.999}'
...
> **Tip:** The transaction returned is encoded in hexadecimal encoding. You can use `boncoin-cli decoderawtransaction` or `boncoin-tx -json` to convert the content to JSON format.

##### signrawtransaction

Before sending a transaction, it must be signed by the private key that the address was derived from. Boncoin Core will automatically use the correct private key when spending UTXO known to the wallet.

```console
#Syntax
satoshi:~$ boncoin-cli signrawtransaction encoded_transaction

#Example
satoshi:~$ boncoin-cli signrawtransaction "01000000013324faf8f03695261611669f5bdd93b68a86cf02db12dc46642ed50666ed69b80000000000ffffffff0200a5459b010000001976a91418a89ee36293f15c4db4c01173babd579243161188ac60b8c4b8000000001976a914c6977da37560e1432c2e14e16952981a4c272cac88ac00000000"
{
...
```console
#Syntax
satoshi:~$ boncoin-cli sendrawtransaction signed_transaction

#Example
satoshi:~$ boncoin-cli sendrawtransaction 
...
> **Note:** To be able to query transactions not related to your own wallet, like in this example, you will need to enable the `-txindex` option. This options requires the Boncoin Core software to re-index the entire blockchain, and can take up to several hours.

First, request the information about block 69:

```console
#Find block hash from his height
satoshi:~$ boncoin-cli getblockhash 69
3d2def20cd0d3aca148741ef469bda11647a3040d7669c82745d03c728706a8b

#Get block data
satoshi:~$ boncoin-cli getblock 3d2def20cd0d3aca148741ef469bda11647a3040d7669c82745d03c728706a8b
{
...
```console
#Syntax
satoshi:~$ boncoin-cli getrawtransaction txid verbose

#Example
satoshi:~$ boncoin-cli getrawtransaction 695ce4208fa7a87ef9e99805b0910dc129058ecdceb5cef7e25f71dcdc7936db 1
{
...
Using `boncoind -help` will display all available configuration parameters that can be added as arguments:

**Command example :**
```console
satoshi:~$ boncoind -daemon -paytxfee=0.01 -sendfreetransactions=1 -maxconnections=150
```

Configuration can be persisted by creating a `boncoin.conf` file. Create it in the directory defined with the `datadir` setting, `$HOME/.boncoin` by default, or specify the file location with `-conf`.

**boncoin.conf example :**
```
daemon=1
server=1
listen=1
...
maxconnections=150
```
You can see a more concrete example [here](/contrib/debian/examples/boncoin.conf).

### Mainnet, testnet and regtest

When trying out new things, for example to test your application that interacts with the Boncoin chain, it is recommended to not use the main Boncoin network. Multiple networks are built-in for this purpose.
...
When not specifying any network, *Mainnet* is the network used by default. To enable *testnet*, use the `boncoind -testnet`.

To enable *regtest*, use the `-regtest` option.

> **Tip:** Remember to specify the network when you want to use `boncoin-cli`.

### Data directory

The data directory is the location where Boncoin Core files are stored, including the wallet, log files and blocks. You can modify the location with the `-datadir` setting.
...
Running Boncoin Core can require a lot of memory, so in some situations it may be necessary to optimize its usage. You can find more information about reducing the memory footprint in the [related guide](reduce-memory.md).

### Troubleshooting

By default, Boncoin Core keeps detailed logs in the `debug.log` file, located in the `datadir`. Alternatively, the `-printtoconsole` parameter displays the log interactively to the terminal instead.
